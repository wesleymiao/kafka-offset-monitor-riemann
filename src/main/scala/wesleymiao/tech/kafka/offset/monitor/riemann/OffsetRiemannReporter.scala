...
class OffsetRiemannReporter (pluginsArgs: String) extends com.quantifind.kafka.offsetapp.OffsetInfoReporter {
  ...
  override def report(info: scala.IndexedSeq[OffsetInfo]) =  {
    ...
    try {
      ...
      val groupedReduced = grouped.mapValues (events => events.map(i => (i.logSize, i.offset, i.lag, i.modified)).reduce( (a, b) => (a._1 + b._1, a._2 + b._2, a._3 + b._3, if (a._4.inSeconds >= b._4.inSeconds) a._4 else b._4)))
      val env = RiemannReporterArguments.env
      val events = groupedReduced.flatMap {
        case ((topic, group), (logSize, offset, lag, mtime)) =>
          val isActive = if ((Time.now - mtime).inSeconds > 1.day.inSeconds) "inactive" else "active"
          val inactiveFor = (Time.now - mtime).inSeconds.toString()
